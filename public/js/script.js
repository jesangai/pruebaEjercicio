...
function listarM(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#avatar").attr("src",data.avatar);
            $.each(data, function(i, result){
            $("#listmascota").append("<div class='col-md-4 col-sm-4'><div><button onclick='pasaralumnomascota("+result.id+")'><img class='listaimg' width='250px' src='"+result.foto+"'/><h1>"+result.nombre+"</h1><h1>"+result.edad+"</h1><h1>"+result.idalumno+"</h1></button></div></div>");
        });
        },
        error: function (data) {
           // alert("error");
...
function listarMascotas(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#listarmascotas").append("<div class='col-md-4 col-sm-4'><div><h1>"+res.id+"</h1><h1>"+res.nombre+"</h1><img class='listaimg' src='"+res.foto+"'/><h1>"+res.edad+"</h1><h1>"+res.idalumno+"</h1><button onclick='pasareditarmascota("+res.id+")'>Editar</button><button onclick='eliminarMascota("+res.id+")'>Eliminar</button></div></div>");
        });
        },
        error: function (data) {
            //alert("error");
...
function cargarmascota(){
    ...
    $.ajax({
        ...
        success: function (data) {
             $.each(data, function(i, res){
            $("#idalumno").append("<option>"+res.id+"</option>");
...
function editarmascota(){
    var id=localStorage.getItem("editaridMascota");
    $.ajax({
        url: '/fcargarmascotaporid',
        type: 'POST',
        data: {
            idmascota:id
        },
        cache: false,
        success: function (data) {
            $("#nombre").attr("value",data[0].nombre);                        $("#divAvatar").html("<img id='avatar' src='"+data[0].foto+"' width='150px'></img>");
            $("#edad").attr("value",data[0].edad);
        //   $("#idalumno").html("<option>"+data[0].idalumno+"</option>");
      console.log(data[0].idalumno);
            $.ajax({
                url: '/fcargarmascota',
                type: 'GET',
                cache: false,
                ...
                success: function (num) {
                     $.each(num, function(i, res){
                     if(parseInt(res.id) == parseInt(data[0].idalumno)){
                          $("#idalumno").append("<option selected='true'>"+res.id+"</option>");
                       } else{
                           $("#idalumno").append("<option>"+res.id+"</option>");
                            }
...
function cargarmascotaalumno(){
   
    formData={
        idmascota:localStorage.getItem("varPasar")
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#nombreanimal").html(data[0].nombre);
            $("#edad").html(data[0].edad);
...
function pasaralumnomascota(pos){
    localStorage.setItem("varPasar",pos);
    window.location = "/leermascotaAlumno";
}
function pasarAlumno(idalumno){
    localStorage.setItem("varidAlumno",idalumno);
    window.location = "/menual";
}
...
function pasareditarmascota(idmascota){
    localStorage.setItem("editaridMascota",idmascota);
    window.location = "/editarmascota";
}
...
function eliminarMascota(posmascota){
        $.ajax({
            url: '/feliminarmascota',
            method: 'POST',
            data: {
                "idmascota": posmascota
            },
            success: function (data) {
                   window.location.href="/menumascotas";
            },
            error: function (data) {
              //  alert("error");
                console.log(data);
...
$('#').click(function () {
            ...
            $.ajax({
                ...
                data:formData,
                success: function (data) {
                   window.location.href="/menuusalumnos";
                }
            });
});
